...
type Props = {
  ...
  /**
   * 'debug' (default) = full diagnostic bar (readouts + all tests) for ?debug=1.
   * 'operator' = only the operational sound picker + test + one OS test, for the
   * settings panel. Same sound state/localStorage either way (notifySound.ts).
   */
  variant?: 'operator' | 'debug';
};

export default function ChatNotifyDiagBar({ onRequestPermission, variant = 'debug' }: Props) {
  ...
  return (
    <section
      className={
        isDebug
          ? 'border-b border-amber-500/80 bg-amber-950/90 px-3 py-2 text-xs text-amber-50'
          : 'px-1 py-1 text-xs text-amber-50'
      }
      aria-label={isDebug ? '알림 진단' : '알림음 설정'}
    >
      {isDebug ? (
        <>
