...
pub fn run() {
    ...
    builder
        ...
        .setup(|app| {
            ...
            // Standard Windows-style app menu: a top-level "AutoFlow" menu with
            // the refresh action + quit. Always visible, discoverable, and
            // web-independent (works on any /chat bundle, no web deploy needed).
            ...
            let win_quit_i = MenuItem::with_id(app, "win_quit", "종료", true, None::<&str>)?;
            let sep = PredefinedMenuItem::separator(app)?;
            let app_sub = Submenu::with_items(
                app,
                "AutoFlow",
                true,
                &[&win_refresh_i, &sep, &win_quit_i],
            )?;
            let window_menu = Menu::with_items(app, &[&app_sub])?;
            ...
            win.on_menu_event(move |win, event| match event.id.as_ref() {
                ...
                "win_quit" => win.app_handle().exit(0),
                _ => {}
            });
